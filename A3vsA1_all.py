...
d_short = {'Comp_name':df_ic50.index,'Scaffold':df_ic50['Scaffold'],'R3':df_ic50['R3'],'PDI_A1':df_ic50['PDI A1'],'PDI_A3':df_ic50['PDI A3'],'IC50':df_ic50_subtNaN}
...
for num in range(len(df_short_sorted['IC50'])):
	if df_short_sorted['IC50'][num] > lim_canc:
		drop_list.append(num)
...
for num in range(len(df_short_sorted['IC50'])):
	if df_short_sorted['Scaffold'][num] == 'S3' or df_short_sorted['Scaffold'][num] == 'S2':
		drop_list.append(num)
...
suptitle = ('Correlation between antiproliferative effect against '+
			str(data_set_dict[cancer_cell_line_no][1])+
			'\nand inhibiting activity against PDI subclasses\n'+
			'(points with value of antiproliferatory effect > 30 - excluded)')
...
for num in range(len(df_droped)):
...
ax2.legend([b],['Scaffold 1'],loc=1)
#ax2.legend([b,r,g],['Scaffold 1','Scaffold 2','Scaffold 3'],loc=1)
file_name = data_set_dict[cancer_cell_line_no][1]+' all_plot.png'
#plt.savefig(file_name)

print(len(df_droped['PDI_A3']))
